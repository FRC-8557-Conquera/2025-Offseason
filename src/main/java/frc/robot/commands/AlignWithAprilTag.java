...
public class AlignWithAprilTag extends Command {
    ...
    private static final double VISION_DES_RANGE_m = 0.3;
    ...
    public AlignWithAprilTag(Swerve swerve, PhotonCamera camera, int targetAprilTagID) {
        ...
        public void execute() {
            ...
            if (!results.isEmpty()) {
                ...
                if (result.hasTargets()) {
                    for (var target : result.getTargets()) {
                        if (target.getFiducialId() == targetAprilTagID) { // AprilTag ID 21, Reef’deki hedef
                            targetYaw = target.getYaw() ; // Kameranın hedefe göre yana dönüşü
                            // Mesafe hesaplaması:
            ...
            if (targetVisible) {
                ...
                // Yana kaydırma komutu: Hedefin açısal sapması 0°'ye (VISION_DES_ANGLE_deg) göre hesaplanır.
            }
